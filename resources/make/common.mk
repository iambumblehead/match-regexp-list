...
ERL_LIBS=.:..:../match-regexp-list:$(shell $(LFETOOL) info erllibs)
OS := $(shell uname -s)
ifeq ($(OS),Linux)
        HOST=$(HOSTNAME)
endif
...
get-version:
	...
	-eval "lfe_io:format(\"~p~n\",['match-regexp-list-util':'get-versions'()])." \
...
install: compile
	@echo "Installing match-regexp-list ..."
	@PATH=$(SCRIPT_PATH) lfetool install lfe

upload: $(EXPM) get-version
	@echo "Preparing to upload match-regexp-list ..."
	@echo
	@echo "Package file:"
	@echo
	@cat package.exs
